...
//   echo "ahjkfhkja";
   if (!(isset($_GET['brand']) and isset($_GET['model']))) {
      ...
      $modelNameSql = "SELECT model_name FROM model where slug = '$modelSlug'";
      $queryModelName = mysqli_query($connect,$modelNameSql);
      $resModelName = mysqli_fetch_array($queryModelName);
...
<html lang="en">
   <head>
    ...
    //   include "../keywords.php";
       $getModel = "SELECT * FROM model where (brand ='$brandName' and slug = '$modelSlug') and isParent = 1 and category = 'mobile' ORDER BY id desc";
      ...
      <meta property="og:title" content="Sell <?php
         $modelNameTxt = explode('-', $_GET['model']);
         for ($i = 0; $i < count($modelNameTxt); $i++) {
             echo $showModelName = ucfirst($modelNameTxt[$i]) . ' ';
         }
         ?> Online For Best Price | Instant Cash & Free Pickup" />
      <meta property="og:description" content="<?php echo $dsc;?>Sell <?php
         $modelNameTxt = explode('-', $_GET['model']);
         for ($i = 0; $i < count($modelNameTxt); $i++) {
             echo $showModelName = ucfirst($modelNameTxt[$i]) . ' ';
         }
         ...
         ?> and free home pickup when you sell your old phone at cashkar." />
   ...
   <body>
      ...
      <div class="container p-0">
         <h3 class="txt-underline p-1 font-size">
             <a href="https://cashkar.in" class="text-decorataion-none text-dark">
                 Home</a> >
                <span class="breadCrums"><a href="https://cashkar.in/sell-laptop/" class="text-decorataion-none text-dark">Select Brand </a></span> > <span class="breadCrums"><a href="https://cashkar.in/select-laptop-brand/<?php echo $_GET['brand'];?>" class="text-decorataion-none text-dark">Select Model </a></span>><span class="breadCrums"> Select Variant</span>
         </h3>
         <div class="col-12 mx-auto mt-5 row">
            <?php
               $getModelData = "SELECT * FROM laptopmodel where slug = '$modelSlug' and brand = '$brandName'";
            ...
            <div class="col-lg-4 col-md-4 col-sm-12 p-1">
               <img src="https://cashkar.in/images/model/laptop/<?php echo $img; ?>" alt="<?php echo $alt; ?>" class="img-fluid">
            </div>
               ...
               <div class="col-lg-11 col-sm-12 m-auto p-0">
                  ...
                  <div class="d-flex center flex-wrap m-auto row p-0">
                     ...
                     <div class="col-12">
                        ...
                        <label for="sel1">Select Hard Disk Storage:</label>
                        <select class="form-control" onchange="checkVariantPrice(this.value)" style="font-weight:700;" id='hard-opt'>
                            <?php 
                            while($resModelData = mysqli_fetch_array($queryModelData)){
                                 $storageSize = $resModelData['storage'];
                                $hardDisk = $storageSize;
                                $showStorage = $storageSize . 'GB';
                                
                            ?>
                           <option value="<?php echo $hardDisk; ?>" style="font-weight:700;"><?php echo $showStorage; ?></option>
                           <?php
                        } 
                        ?>
                        </select>
                     ...
                     <div class="col-12">
                         <div class="form-group">
                        <label for="sel1">Select RAM Size:</label>
                        <select class="form-control" onchange="checkVariantPrice(this.value)" style="font-weight:700;" id='ram-opt'>
                            <?php 
                            $getModelData = "SELECT * FROM laptopmodel where slug = '$modelSlug' and brand = '$brandName'";
                        $queryModelData = mysqli_query($connect, $getModelData);
                            while($resModelData = mysqli_fetch_array($queryModelData)){
                            ...
                            ?>
                           <option value="<?php echo $ram; ?>" style="font-weight:700;"><?php echo $showRAM; ?></option>
                           <?php
                        } 
                        ?>
                        </select>
                     ...
                     <div class="col-12">
                         <div class="form-group">
                        <label for="sel1">Select Processer:</label>
                        
                        <select class="form-control" onchange="checkVariantPrice(this.value)" style="font-weight:700;" id='processer-opt'>
                            <?php 
                            $getModelData = "SELECT * FROM laptopmodel where slug = '$modelSlug' and brand = '$brandName'";
                        $queryModelData = mysqli_query($connect, $getModelData);
                            while($resModelData = mysqli_fetch_array($queryModelData)){
                                 $processer = $resModelData['processer'];
                                
                            ?>
                           <option value="<?php echo $processer; ?>" style="font-weight:700;"><?php echo $processer; ?></option>
                           <?php
                        } 
                        ?>
                        </select>
      ...
      <script>
         function checkVariantPrice() {
             var ram = document.getElementById('ram-opt').value;
            //  console.log(ram);
            var hardDisk = document.getElementById('hard-opt').value
             var processer = document.getElementById('processer-opt').value;
             var ajaxVariationSwitch = new XMLHttpRequest();
             ajaxVariationSwitch.open('GET', 'https://cashkar.in/ajax/variationChangePrice.php?brand=<?php echo $brandName; ?>&c=laptop&slug=<?php echo $modelSlug; ?>&ram='+ram+'&hd='+hardDisk+'&processer='+processer, 'TRUE');
