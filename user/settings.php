...
if (!(isset($_SESSION['USER-LOGIN']))) {
  ...
  $sqlUserData = "SELECT * FROM users where id = '$userID'";
...
if (isset($_POST['updateData'])) {
  ...
  $sqlOldPw = "SELECT * FROM users where id=$userID and `password` = '$oldPw'";
  $queryOldPw = mysqli_query($connect, $sqlOldPw);
  if (mysqli_num_rows($queryOldPw) > 0) {
    if(!empty($newPw) and strlen($newPw) > 6){
      if ($newPw === $confirmNew) {
        $newPw = md5($newPw);
        $sqlUpdatePw = "UPDATE users SET `password` = '$newPw' where id = '$userID'";
